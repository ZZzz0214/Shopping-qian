...
const handleCloseValue = (index: number, valueIndex: number) => {
  ...
  //
  // const removedValue = attributeList.value[index].values?.[valueIndex]; // 记录删除的那一项数据
  // console.log('删除的属性值:', JSON.stringify(removedValue, null, 2));
  // console.log('删除后属性值:',JSON.stringify(attributeList.value[index], null, 2))
  ...
  emit('update:property-list', attributeList.value) // 发送更新事件
...
/** 输入框失去焦点或点击回车时触发 */
const emit = defineEmits(['success'],['update:property-list']) // 定义 success 事件，用于操作成功后的回调

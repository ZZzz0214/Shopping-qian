    ...
    >
      ...
      <template #file="{ file }">
        ...
        <div class="upload-handle" @click.stop>
          <div v-if="!disabled" class="handle-icon" @click="handleChange(file)">
            <Icon icon="ep:edit" />
            <span>更改</span>
          </div>
...
const handleChange = (file) => {
  ...
  input.type = 'file'
  input.accept = props.fileType.join(',')
  input.onchange = (event) => {
    ...
    if (newFile) {
      ...
      if (index !== -1) {
        ...
        fileList.value.splice(index, 1, {
          name: newFile.name,
          url: URL.createObjectURL(newFile)  // 生成新的URL
        })
        beforeUpload(newFile)  // 调用上传前的校验
      }
    }
  }
  input.click()

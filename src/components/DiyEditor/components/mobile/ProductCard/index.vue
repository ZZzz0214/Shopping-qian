...
<!--          佣金 -xz -->
          <!-- 佣金 -->
          <span
            v-if="property.fields.commission.show && spu.commission"
            class="ml-4px text-10px"
            :style="{ color: property.fields.commission.color }"
          >
  佣金：￥{{ spu.commission }}
</span>
        </div>
...
watch(
  ...
  async () => {
    ...
    spuList.value.forEach((spu) => {
        spu.price = formatToFraction(spu.price)
        spu.marketPrice = formatToFraction(spu.marketPrice)
        spu.commission = formatToFraction(spu.commission)  // 处理佣金字段
